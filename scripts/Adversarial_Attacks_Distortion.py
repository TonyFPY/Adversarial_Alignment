...
def execute_attack(model, img, target, eps, alpha):
    ...
    # perturbed_img = attack_obj.l2_pgd_attack(model, img, target, eps=eps, alpha=0.1, iters=5)
    perturbed_img = attack_obj.l2_pgd_attack(model, img, target, eps=eps, alpha=1, iters=3)
...
def main():
    ...
    parser.add_argument("-m", "--model",
                        ...
                        choices=[
                            ...
                            "swin", "mixnet", "dpn", "darknet", "maxvit", 
                            "volo", "deit", "nfnet", "cait", "xcit", "tinynet", "lcnet", "dla", 
                            "mnasnet", "coatnet", "csp", "eva", "tf",
                            "0", "1", "2", "3", "4",
                        ],
    ...
    for i, model_name in enumerate(model_names[offset:]):
        ...
        for imgs, hmps, labels in data.get_dataset(2 , False): # image, heatmap, label
            ...
            for img, hmp, label in zip(imgs, hmps, labels):
                ...
                perturbed_img, perturbed_pred = execute_attack(model=model, img=img, target=target, eps=initial_eps, alpha=alpha)
                ...
                while l < r:
                    m = l + (r - l) // 2
                    eps = epsilons[m]
                    perturbed_img, perturbed_pred = execute_attack(model=model, img=img, target=target, eps=eps, alpha=alpha)
                    if perturbed_pred.item() == target.item():
                        l = m + 1
                    else:
                        r = m
                ...
                if not optimal_eps in info:
                    perturbed_img, _ = execute_attack(model=model, img=img, target=target, eps=optimal_eps, alpha=alpha)
                else:
                ...
                # Save the data into 
                row_data = [
                    model_name, str(total_cnt-1), str(round(optimal_eps, 6)), 
