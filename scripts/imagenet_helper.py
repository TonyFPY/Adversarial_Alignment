...
def common_superclass_wnid(group_name):
    ...
    common_groups = get_common_groups()
...
# Function to find the superclass ID and label
def find_superclass_info(imagenet_id, mapper):
    for superclass_id, (label, id_set) in mapper.items():
        if imagenet_id in id_set:
            return superclass_id, label
    return -1, "Other"  # or some defaults

# Function to map ID to superclass IDs and labels for all mappers
def map_to_superclasses(imagenet_id, all_mappers):
    result = []
    for mapper in all_mappers:
        superclass_id, label = find_superclass_info(imagenet_id, all_mappers[mapper])
        result.extend([superclass_id, label])
    return result

if __name__ == '__main__':
    ds_path = '/media/data_cifs/imagenet'
    ds_info_path = '/media/data_cifs/pfeng2/Harmoization/datasets/imagenet_label'
    IH = ImageNetHierarchy(ds_path, ds_info_path)

    # in_wnids, wnid_to_name, wnid_to_num, num_to_name = IH.load_imagenet_info(ds_path, ds_info_path)

    '''
    superclass_wnid = common_superclass_wnid('living_9') #common_superclass_wnid('big_12'))
    print(superclass_wnid)

    class_ranges, label_map = IH.get_subclasses(list(superclass_wnid))
    print(class_ranges)
    print(label_map)
    '''
    ...
    superclass_wnid = set()
    common_groups = get_common_groups()
    for key in common_groups:
        superclass_wnid.update(set(common_groups[key]))
    print(superclass_wnid)
    print(len(superclass_wnid))
    '''
    
    common_groups = ['living_9', 'mixed_10', 'mixed_13', 'geirhos_16', 'big_12', 'mid_12']
    superclass_mapper = {}
    for common_group in common_groups:
        superclass_wnid = common_superclass_wnid(common_group)
        subclasses, superclass_dict = IH.get_subclasses(list(superclass_wnid))
        
        for superclass_id, superclass_label in superclass_dict.items():
            if common_group not in superclass_mapper: superclass_mapper[common_group] = {}
            superclass_mapper[common_group][superclass_id] = (superclass_label, list(subclasses[superclass_id]))
    
    print(superclass_mapper['big_12'])
    
    # for common_group in common_groups: 
    #     print(common_group)
    #     print(list(superclass_mapper[common_group].values()))
    #     print()
